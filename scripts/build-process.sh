...
# This script builds CRRCSim for Mac OS.
#
# CREDITS: Instructions heavily inspired by:
...
mkdir -p ~/repos/
cd ~/repos/
wget --no-verbose https://downloads.sourceforge.net/project/crrcsim/crrcsim/crrcsim-0.9.12/crrcsim-0.9.12.tar.gz
tar -zxf crrcsim-0.9.12.tar.gz
rm crrcsim-0.9.12.tar.gz
mv crrcsim-0.9.12/ crrcsim
cd ~/repos/crrcsim
...
# http://blog.mywarwithentropy.com/2012/03/how-to-compile-crrcsim-v0912-for-mac.html
# ...but has been modified to work on OSX Mavericks.
cd ~/repos/crrcsim
patch -p1 -R << 'EOF'
...
cd ~/repos/crrcsim
CC=gcc-mp-4.7 CXX=g++-mp-4.7 ./configure CPPFLAGS="-I/opt/local/include -DAPIENTRY=" LDFLAGS="-L/opt/local/lib -lintl -lgmp -framework OpenGL -headerpad_max_install_names" --prefix=$HOME/crrcsim.app --verbose --disable-sdltest
